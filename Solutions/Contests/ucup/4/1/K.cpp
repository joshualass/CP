...
struct Lazy {
    ...
    static constexpr D ln = {an, 0}; //lazy neutral, applying this value to its node will not change its value
    ...
    bool F(T treeval, T targetval) {return treeval >= targetval;}
...
signed main() {
    ...
    Lazy<ll, array<ll,2>> lazy(n);

    for(int i = 0; i < n; i++) {
        lazy.update(i, i + 1, {d, 0});

        int h = a[i];

        int p0 = 0;
        int p1 = lazy.findFirst(0, i + 1, h);
        int p2 = lazy.findFirst(0, i + 1, h * 2);
        int p3 = i + 1;

        //no change p0 p1
        //assign p1 p2
        lazy.update(p1, p2, {h - 1, 0});
        //subtract h p2 p3
        lazy.update(p2, p3, {an, -h});

        for(auto [l, qid] : qs[i]) {
            ll ans = lazy.query(l, l + 1);
            res[qid] = ans;
        }
    }
