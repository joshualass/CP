...
struct RMQ  {
    ...
    int op(int x, int y) { //update this method to determine what value we are trying to find. Currently set to minimum (return index of minimum element)
        return elements[x] < elements[y] ? y : x;
    }
...
struct RMQMIN  {
    ...
    int most_significant_bit_index(int x) {return 31 - __builtin_clz(x);}
    int small_query(int r, int size = block_size) {return r - most_significant_bit_index(mask[r] & ((1<<size)-1));}
    RMQMIN() {} //need this to satisfy master goon's requirements
    RMQMIN (const vector<T>& input) {build(input);}
    void build (const vector<T>& input) {
        elements = input;
        n = input.size();
        mask.assign(n,0);
...
signed main() {
    ...
    RMQ<int> rmqb(b);
    vector<int> eb(n * 2); //eb[i] => 
    for(int i = 0; i < n * 2; i++) {

        eb[i] = rmqb.query(i, min(n * 2 - 1, i + k));
    }
    ...
    for(int qq = 0; qq < q; qq++) {
        ...
        l--; r--;
        if(r < l) r += n;
        int time = min(b[l], b[r]);
        if(r - l >= k) time = min(time, rmqeb.query(l, r - k));
        time += l;
        cout << (time <= 0 ? -1 : time) << '\n';
    }
