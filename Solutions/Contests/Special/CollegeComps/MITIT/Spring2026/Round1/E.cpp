...
during the contest, i initially had the correct approach of doing some dp state with the snake. 
i had dp[i][j] store having a snake at time i be length j having total score. this worked, but it 
        ...
        for(int len = 2; len <= n + 1; len++) {
            ...
            for(int i = 1; i <= n; i++) {
                if(maxlen[i] >= len && len > 1 && pc[i][len-1] < i) {
                    ...
                    nx.update(i, cur.query(pc[i][len-1], i) + (len - 1) * (i + 1) - (i + 1) * len + 1);
                }
                // res = max(res, dp[i][len] + (n + 1) * len);
            }
            res = max(res, nx.query(0, n + 1) + (n + 1) * len);
            swap(cur, nx);
        }
    }
