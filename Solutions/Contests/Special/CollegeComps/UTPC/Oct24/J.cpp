...
template<typename flow_t = int, typename cost_t = ll>
struct MinCostFlow {
    struct Edge {
        cost_t c;
        flow_t f;
...
signed main() {
    ...
    MinCostFlow<int,ll> g(5 + used_machines);
    int aidx = used_machines, bidx = used_machines + 1, cidx = used_machines + 2, source = used_machines + 3, sink = used_machines + 4;

    int t = 0;
    for(int i = 0; i < n; i++) {
        if(a[i][3]) {
            g.addEdge(t,aidx, 1, a[i][0]);
            g.addEdge(t,bidx, 1, a[i][1]);
            g.addEdge(t,cidx, 1, a[i][2]);
            g.addEdge(source,t,1,0);
            t++;
        }
    }

    g.addEdge(aidx,sink,x,0);
    g.addEdge(bidx,sink,y,0);
    g.addEdge(cidx,sink,z,0);

    auto [flow, cost] = g.minCostMaxFlow(source, sink);
