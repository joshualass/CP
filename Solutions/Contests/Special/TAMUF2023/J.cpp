...
signed main() {
    ...
    SegtreeLazy<ll> prefx(1e6 + 2, 0, 0, modifyElement, modifyRange, queryType);
    SegtreeLazy<ll> suffx(1e6 + 2, 0, 0, modifyElement, modifyRange, queryType);
    SegtreeLazy<ll> prefy(1e6 + 2, 0, 0, modifyElement, modifyRange, queryType);
    SegtreeLazy<ll> suffy(1e6 + 2, 0, 0, modifyElement, modifyRange, queryType);
    for(int i = 0; i < n; i++) {
        int a, b; cin >> a >> b;
        prefx.modify(a, 1e6 + 1, 1);
        suffx.modify(0, a, 1);
        prefy.modify(b, 1e6 + 1, 1);
        suffy.modify(0, b, 1);        
    }
    ...
    for(int i = 0; i < q; i++) {
        ll x, y; cin >> x >> y;
        ll ans = 0;
        ans += suffx.query(x, 1e6 + 1);
        ans += prefx.query(1, x);
        ans += suffy.query(y, 1e6 + 1);
        ans += prefy.query(1, y);
        cout << ans * 2 << '\n';
    }
