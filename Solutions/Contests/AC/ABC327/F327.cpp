...
signed main() {
    ...
    int n, d, w; cin >> n >> d >> w; //bounds of rectangle

    vector<vector<pair<int,int>>> apples(N);

    for(int i = 0; i < n; i++) {
        int t, x; cin >> t >> x;
        t--; x--;
        apples[t].push_back({x, x + w});
    }
    ...
    SegtreeLazy<ll> tree(N * 3,0,0,modifyElement,modifyRange,queryType);
    ll ans = 0;
    for(int t = 0; t < N; t++) {
        if(t - d >= 0) {
            for(pair<int,int> p : apples[t-d]) {
                tree.modify(p.first,p.second,-1);
            }
        }
        for(pair<int,int> p : apples[t]) {
            tree.modify(p.first,p.second,1);
        }
        ans = max(ans, tree.query(0,N));
    }
    cout << ans << '\n';
    return 0;
}