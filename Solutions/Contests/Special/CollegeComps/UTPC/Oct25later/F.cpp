...
void solve() {
    ...
    ll w, h, x1, y1, x2, y2; cin >> w >> h >> x1 >> y1 >> x2 >> y2;
    vector<ll> A, M;
    A.push_back(w * (y1 + y2));
    M.push_back(2 * w * (y1 + y2));

    A.push_back(h * (x1 + x2));
    M.push_back(2 * h * (x1 + x2));

    cout << "A : " << A << '\n';
    cout << "M : " << M << '\n';

    auto [resA, resM] = chinese_remainder_theorem(A, M);

    cout << "resA : " << resA << " resM : " << resM << '\n';

    if(resA == -1) {
        cout << "-1\n";
    } else {
        cout << fixed << setprecision(10) << ((ld) resA) / ((x1 + x2) * (y1 + y2)) << '\n';
    }
