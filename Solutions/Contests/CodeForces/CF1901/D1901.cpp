...
signed main() {
    ...
    int ans = INT_MAX;
    for(int i = 0; i < n; i++) {
        int leftside = hiRMQ.query(0, i) - (n-i-1);
        int rightside = (i == n - 1 ? 0 :loRMQ.query(i + 1,n-1));

        // cout << "i1 : " << i << " ls : " << leftside << " rs : " << rightside << '\n';
        // ans = min(ans, max(leftside, rightside));

        int rightside1 = loRMQ.query(i,n-1) - i;
        int leftside1 = (i == 0 ? 0 : hiRMQ.query(0,i-1));

        // cout << "i2 : " << i << " ls : " << leftside1 << " rs : " << rightside1 << '\n';
        ans = min(ans, max({leftside, rightside, leftside1, rightside1}));
    }
