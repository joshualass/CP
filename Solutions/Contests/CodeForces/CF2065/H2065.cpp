...
void solve() {
    ...
    vector<Tree<Z>> pre(2, Tree<Z>(n)), suf(2, Tree<Z>(n));
    ...
    auto update = [&](int i) -> void {
        ...
        int j = s[i] == '1';
        pre[j].update(i, power<Z>(2, i));
        suf[j].update(i, power<Z>(2, n - 1 - i));
    };

    auto get_left_score = [&](int i) -> Z {
        int j = s[i] == '0';
        return pre[j].query(0, i) * power<Z>(2, n - i - 1);
    };

    auto get_right_score = [&](int i) -> Z {
        int j = s[i] == '0';
        return suf[j].query(i + 1, n) * power<Z>(2, i);
    };

    auto get_score = [&](int i) -> Z {
        return get_left_score(i) + get_right_score(i);
    };
