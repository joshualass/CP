...
struct Lazy {
    static constexpr T qn = 1e18; // query neutral, when we query, doing the operation with this value won't change our query
...
signed main() {
    ...
    ll n, c;
    cin >> n >> c;
    Lazy<ll, ll> lazy_pos(n, 0);
    Lazy<ll, ll> lazy_neg(n, 0);
    for(int i = 0; i < n; i++) {
        ll a;
        cin >> a;
        ll ans = 0;
        ans = max(ans, a - lazy_pos.query(0, i));
        ans = max(ans, -a - lazy_neg.query(0, i));
        lazy_pos.update(i, i + 1, a);
        lazy_neg.update(i, i + 1, -a);
        lazy_pos.update(0, i + 1, c);
        lazy_neg.update(0, i + 1, c);
        cout << ans << " ";
    }
    cout << "\n";
