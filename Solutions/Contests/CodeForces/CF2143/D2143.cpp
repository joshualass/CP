...
void solve() {
    ...
    vector r(n + 1, Tree<Z>(n + 1)), c(n + 1, Tree<Z>(n + 1));
    vector b(n + 1, vector<Z>(n + 1));

    auto add = [&](int i, int j, Z x) -> void {
        // cout << "add i : " << i << " j : " << j << " x : " << x << '\n';
        r[i].update(j, x);
        c[j].update(i, x);
        b[i][j] += x;
    };
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < cur; j++) {
            add(cur, j, c[j].query(0, cur + 1));
        }

        for(int i = cur + 1; i <= n; i++) {
            add(i, cur, r[i].query(0, cur + 1));
        }
        // cout << "i : " << i << " b\n" << b;
    }

    Z res = 0;
    for(int i = 0; i <= n; i++) res += r[i].query(0, n + 1);
    cout << res << '\n';
