...
void solve() {
    ...
    rotate(v.begin(), v.begin() + zeroidx, v.end());
    // cout << v << '\n';
    map<ll,ll> idxs;
    for(int i = 0; i < n; i++) {
        idxs[v[i]] = i;
    ...
    for(int i = 1; i < n; i++) {
        ll mex = v[i];
        if(mexs.upper_bound(mex) != mexs.end()) {
            auto it = mexs.upper_bound(mex);
            ll lo = (*it).second;

            // cout << "i : " << i << " mex : " << mex << " lo : " << lo << '\n';
            ll hi = i;
            tree.modify(lo,hi,mex);
            while(mexs.upper_bound(mex) != mexs.end()) {
                mexs.erase(mexs.upper_bound(mex));
            }
            mexs[mex] = lo;
        } else {
            tree.modify(i-1,mex);
