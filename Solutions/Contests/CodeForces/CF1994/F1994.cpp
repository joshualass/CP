...
// worth solving because i solved a problem that used a similar approach. 
...
void solve() {
    ...
    int n, m; cin >> n >> m;
    vector<multiset<int>> adjgraph(n), adj(n);
    int cnt = 0;
    ...
    auto dfs = [&](auto self, int i) -> void {
        while(adj[i].size()) {
            ...
            self(self, child);
            if(adjgraph[child].size() & 1) {
                adjgraph[i].insert(child);
                adjgraph[child].insert(i);
                cnt++;
            }
        }
    };
    ...
    // for(int i = 0; i < n; i++) {
    //     cout << "i : " << i << " adjg : " << adjgraph[i] << '\n';
    // }

    for(int i = 0; i < n; i++) {
        if(adjgraph[i].size() & 1) {
            cout << "NO\n";
            return;
        }
    }
    ...
    auto dfs2 = [&](auto self, int i) -> void {
        while(adjgraph[i].size()) {
            int child = *adjgraph[i].begin();
            adjgraph[i].erase(adjgraph[i].find(child));
            adjgraph[child].erase(adjgraph[child].find(i));
            self(self, child);
            cout << i + 1 << " ";
        }
    };
