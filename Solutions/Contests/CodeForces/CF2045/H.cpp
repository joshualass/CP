...
signed main() {
    ...
    //dp[i][j] - substring of the interval [i,j)
    vector dp(n + 1, vector<array<int,2>>(n + 1, {-1,-1}));
    dp[0][0] = {0,-1};

    for(int i = 1; i < n; i++) { //current left endpoint of the next string or right endpoint of previous string exclusive
        for(int j = 0; j < n; j++) { //current left endpoint of the previous string

            if(dp[j][i-1][0] > dp[j][i][0]) {
                dp[j][i] = dp[j][i-1];
            }
            ...
            if(dp[j][i][0] >= 0) {
                ...
                // cout << "ok i : " << i << " j : " << j << " val ? " << dp[j][i] << " lcp : " << lcp << '\n';
                if(lcp >= i - j) {
                    sidx = i + (i - j) + 1;
                } else if(sa.isa[i] > sa.isa[j]) {
                    sidx = i + lcp + 1;
                }
                // cout << "sidx : " << sidx << '\n';
                if(sidx <= n && dp[j][i][0] + 1 > dp[i][sidx][0]) {
                    // cout << "j : " << j << " i : " << i << " sidx : " << sidx << '\n';
                    dp[i][sidx] = {dp[j][i][0] + 1, j};
                }
            }
        }
    }
    ...
    int bestprev = 0;
    for(int i = 0; i < n; i++) {
        // cout << "prev last i : " << i << " val : " << dp[i][n][0] << '\n';
        if(dp[i][n][0] > dp[bestprev][n][0]) {
            bestprev = i;
        }
    }

    int lo = bestprev, hi = n;
    vector<string> res;
    while(lo != -1) {
        res.push_back(s.substr(lo, hi - lo));
        int t = dp[lo][hi][1];
        hi = lo;
        lo = t;
    }
