...
signed main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        here += suf[lo] + ((n - k) - (n - lo)) * k;
        here -= s;
        lazy.update(i, i + 1, here);
    }

    vector<int> idxs(n);
    iota(idxs.begin(), idxs.end(), 0);

    sort(idxs.begin(), idxs.end(), [&](const auto &lhs, const auto &rhs) -> bool {
        if(a[lhs] != a[rhs]) {
            return a[lhs] > a[rhs];
        } else {
            return lhs > rhs;
