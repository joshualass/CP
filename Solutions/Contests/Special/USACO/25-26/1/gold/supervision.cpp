...
/*
has a very tight runtime and need a very optimized lazy segtree ... 
...
struct Lazy {
    ...
    void main_comb(int node, int size) {//update this depending on query type, how does the lazy value affect value at v for the query?
        v[node] = v[node] * lazy[node][0] + lazy[node][1] * size;
        // v[node] += lazy[node];
    }
...
signed main() {
    ...
    for(int i = n - 1; i >= 0; i--) {
        ...
        if(b[i] == 0) { // camper
            lazy.update(i, n, {2, 0});
        } else { //coach, compute dp transitions
            Z val = lazy.query(i + 1, n);
            val += power<Z>(2, p[r+1] - p[i]);
            lazy.update(i, i + 1, {0, val});
            res += val;
