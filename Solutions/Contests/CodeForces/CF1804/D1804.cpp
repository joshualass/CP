...
signed main() {
    ...
    //minimize by putting as many doubles on 11. every other 1 will add to a room. 
    auto find_min = [](string s) {
        int dc = 0;
        ...
        return count(s.begin(), s.end(), '1') - min((int) (s.size() / 4), dc);
    };

    //if we see 3+ 0's in a row, we place a double. Then, put as many doubles as possible. 
    auto find_max = [](string s) {
...
//            DDDDDDDDS
//           DDDDDDDDDD
// 00000100011111111111
//
// greedy does not seem good
